...
const { error: e1 } = await supabase.from('participations').delete().eq('restaurant_id', r.id);
...
await charger(); await supabase.from('codes').delete().eq('restaurant_id', r.id); await supabase.from('sous_lots').delete().eq('restaurant_id', r.id); await supabase.from('lots').delete().eq('restaurant_id', r.id); await supabase.from('config').delete().eq('restaurant_id', r.id); await supabase.from('restaurants').delete().eq('id', r.id); charger(); }} style={{padding:'6px 12px',borderRadius:'8px',border:'none',cursor:'pointer',background:'#fee2e2',color:'#dc2626',fontSize:'12px',fontWeight:'bold'}}>
                  🗑️ Suppr
                </button>
              </div>
            </div>
